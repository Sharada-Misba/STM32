...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_SvcHdle;                    /**< MY_SVC handle */
  uint16_t  CustomMy_ReadHdle;                  /**< MY_READ handle */
  uint16_t  CustomMy_NotiHdle;                  /**< MY_NOTI handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeMy_Read = 1;
uint16_t SizeMy_Noti = 1;
...
#define COPY_MY_SVC_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_MY_READ_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_MY_NOTI_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomMy_NotiHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MY_NOTI_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MY_NOTI_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomMy_ReadHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
          ...
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomMy_ReadHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for MY_SVC +
   *                                2 for MY_READ +
   *                                2 for MY_NOTI +
   *                                1 for MY_NOTI configuration descriptor +
   *                              = 6
   *
  ...
  COPY_MY_SVC_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_SvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: MY_SVC, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: MY_SVC \n\r");
  }

  /**
   *  MY_READ
   */
  COPY_MY_READ_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMy_SvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMy_Read,
                          CHAR_PROP_WRITE,
                          ...
                          &(CustomContext.CustomMy_ReadHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MY_READ, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MY_READ \n\r");
  }
   ...
   *  MY_NOTI
   */
  COPY_MY_NOTI_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMy_SvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMy_Noti,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomMy_NotiHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MY_NOTI, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MY_NOTI \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MY_READ:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_SvcHdle,
                                       CustomContext.CustomMy_ReadHdle,
                                       0, /* charValOffset */
                                       SizeMy_Read, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MY_READ command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MY_READ command\n\r");
      }
    ...
    case CUSTOM_STM_MY_NOTI:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_SvcHdle,
                                       CustomContext.CustomMy_NotiHdle,
                                       0, /* charValOffset */
                                       SizeMy_Noti, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MY_NOTI command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MY_NOTI command\n\r");
      }
      /* USER CODE BEGIN CUSTOM_STM_App_Update_Service_1_Char_2*/
      if (ret != BLE_STATUS_SUCCESS)
                       {
               	  	  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1,GPIO_PIN_SET);  // Turn RED LED ON (error)
               	  	  HAL_Delay(500);
                       ...
                       {
               	  	  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5,GPIO_PIN_SET); // Turn BLUE LED ON (success)
               	  	  HAL_Delay(500);
               	  	  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5,GPIO_PIN_RESET);
               	  	  //printf("print1");
               	  	  printf("Data Updated = %d" , *pPayload);
               	  	  //printf("print2");
                       }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MY_READ:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_SvcHdle, CustomContext.CustomMy_ReadHdle, SizeMy_Read, pPayload);
    ...
    case CUSTOM_STM_MY_NOTI:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_SvcHdle, CustomContext.CustomMy_NotiHdle, SizeMy_Noti, pPayload);
