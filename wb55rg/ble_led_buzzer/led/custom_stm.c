...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMydevc_SvcHdle;                    /**< MYDEVC_SVC handle */
  uint16_t  CustomMydevc_CharHdle;                  /**< MYDEVC_CHAR handle */
  uint16_t  CustomNoti_CharHdle;                  /**< NOTI_CHAR handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeMydevc_Char = 1;
uint16_t SizeNoti_Char = 1;
...
#define COPY_MYDEVC_SVC_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_MYDEVC_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_NOTI_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomNoti_CharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_NOTI_CHAR_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_NOTI_CHAR_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomMydevc_CharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for MYDEVC_SVC +
   *                                2 for MYDEVC_CHAR +
   *                                2 for NOTI_CHAR +
   *                                1 for NOTI_CHAR configuration descriptor +
   *                              = 6
   *
  ...
  COPY_MYDEVC_SVC_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMydevc_SvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: MYDEVC_SVC, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: MYDEVC_SVC \n\r");
  }

  /**
   *  MYDEVC_CHAR
   */
  COPY_MYDEVC_CHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMydevc_SvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMydevc_Char,
                          CHAR_PROP_WRITE,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MYDEVC_CHAR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MYDEVC_CHAR \n\r");
  }
   ...
   *  NOTI_CHAR
   */
  COPY_NOTI_CHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMydevc_SvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeNoti_Char,
                          CHAR_PROP_NOTIFY,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : NOTI_CHAR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : NOTI_CHAR \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYDEVC_CHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomMydevc_SvcHdle,
                                       CustomContext.CustomMydevc_CharHdle,
                                       0, /* charValOffset */
                                       SizeMydevc_Char, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MYDEVC_CHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MYDEVC_CHAR command\n\r");
      }
    ...
    case CUSTOM_STM_NOTI_CHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomMydevc_SvcHdle,
                                       CustomContext.CustomNoti_CharHdle,
                                       0, /* charValOffset */
                                       SizeNoti_Char, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value NOTI_CHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value NOTI_CHAR command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYDEVC_CHAR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMydevc_SvcHdle, CustomContext.CustomMydevc_CharHdle, SizeMydevc_Char, pPayload);
    ...
    case CUSTOM_STM_NOTI_CHAR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMydevc_SvcHdle, CustomContext.CustomNoti_CharHdle, SizeNoti_Char, pPayload);
